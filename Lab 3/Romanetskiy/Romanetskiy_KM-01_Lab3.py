...
def variant_16(n):
    print(f"Задача варіаційного числення для N = {n}")
    ...
    plt.title(f"Розв'язки задачі варіаційного числення при N = {n}")
    plt.legend()
    ax = plt.gca()
...
def calculate_solution_without_plot(n):
    x0 = np.array([1 for _ in range(n + 2)])
    res = scipy.optimize.minimize(j, x0)
    ...
    error = mean_squared_error(analytic, y_list)

    return error


def calculate_errors_and_plot(N_values):
    errors = []

    for N in N_values:
        error = calculate_solution_without_plot(N)
        errors.append(error)

    plt.figure()
    plt.plot(N_values, errors, marker='o', color='black')
...
if __name__ == "__main__":
    ...
    # Графіки розв'язків задачі варіаційного числення
    for N in [10, 50, 100]:
        step = (B-A)/N
        variant_16(N)

    #  Графік залежності похибки від N
    calculate_errors_and_plot(range(10, 110, 10))